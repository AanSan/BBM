...
const SPREADSHEET_WEBAPP_URL = "https://script.google.com/macros/s/AKfycbw8J35Rt3bsHAngNE1gI0cnVfhuYBCyV31VFJzMXE-jBtEAUHia7lS45qSAoai9DvUTRw/exec";
...
async function pollAndRetrieve(whisperHash, apiKey) {
    ...
    const statusUrl = `${apiBase}/whisper-status?whisper_hash=${whisperHash}`;
    const retrieveUrl = `${apiBase}/whisper-retrieve?whisper_hash=${whisperHash}`;
    const headers = { "unstract-key": apiKey };
    const scanStatus = document.getElementById('scanStatus');

    while (true) {
        await new Promise(resolve => setTimeout(resolve, 1500));
        if (scanStatus) scanStatus.innerText = "Memproses OCR...";
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const handleFileChange = async (e) => {
        ...
        if (scanStatus) {
            ...
            scanStatus.innerText = "Mengompres gambar...";
        }
        ...
        try {
            ...
            const imageBlob = dataURLtoBlob(fullBase64);
            const apiBase = window.location.port === "8080" ? "/api" : remoteBaseUrl;
            const response = await fetch(`${apiBase}/whisper?mode=high_quality&output_mode=layout_preserving`, {
                method: "POST",
                headers: { "unstract-key": activeApiKey, "Content-Type": "application/octet-stream" },
                body: imageBlob
            });

            const data = await response.json();
            const extractedText = await pollAndRetrieve(data.whisper_hash, activeApiKey);

            if (extractedText) {
                const hasil = parseNotaText(extractedText);
                ...
                if (scanStatus) scanStatus.innerText = "Selesai!";
            }
        } catch (error) {
    ...
    if (formNota) {
        formNota.addEventListener('submit', (e) => {
            ...
            if (SPREADSHEET_WEBAPP_URL) {
                ...
                // Mengirimkan data via POST
                fetch(SPREADSHEET_WEBAPP_URL, {
                    method: 'POST',
                    mode: 'no-cors', // Penting untuk melewati pembatasan CORS Apps Script
                    headers: { 'Content-Type': 'text/plain;charset=utf-8' },
                    body: JSON.stringify(newNota)
                })
                    .then(() => {
                        console.log("Data berhasil dikirim. Memuat ulang data dari Sheets...");

                        // Beri delay 1.5 detik agar Google Sheets selesai menulis baris baru
                        setTimeout(() => {
                            muatDataDariSpreadsheet();
                        }, 1500);

                        // Reset form & foto temporary
                        formNota.reset();
                        tempCurrentBase64 = "";
...
function renderTabel() {
    ...
    // 1. Ambil nilai dari inputan filter
    ...
    filteredData.forEach((item) => {
        ...
        // Di dalam loop renderTabel() pada app.js:
