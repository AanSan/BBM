...
const WHISPER_API_KEY_FALLBACK = "uf3mDFBoCCQNyVhE0XrJPQ_exEH5zlPTXIVGSMCGNjM";
const WHISPER_BASE_URL = "https://llmwhisperer-api.us-central.unstract.com/api/v2";

function getWhisperApiKey() {
    try {
        var key = PropertiesService.getScriptProperties().getProperty("WHISPER_API_KEY");
        if (key && key.trim() !== "") {
            return key.trim();
        }
    } catch (e) { }
    return WHISPER_API_KEY_FALLBACK;
}
...
function sanitizeForSheet(val) {
    ...
    if (typeof val === 'string') {
        var str = val.trim();
        // Cegah Formula/CSV Injection pada Google Sheets
        if (/^[=\+\-@\t\r]/.test(str)) {
            return "'" + str;
        }
        return str;
...
function isAuthorized(e) {
    var expectedToken = PropertiesService.getScriptProperties().getProperty("APP_AUTH_TOKEN");
    if (!expectedToken) return true; // Jika token belum diset di Script Properties, izinkan akses (backward compatible)
...
function doGet(e) {
    try {
        if (!isAuthorized(e)) {
            return createJsonResponse({ status: "error", message: "Akses ditolak: Token otorisasi tidak valid." });
        }
        ...
        if (dataNota.length > 1) {
            var fotoRichText = sheetNota.getRange(2, 5, dataNota.length - 1, 1).getRichTextValues();
            ...
            for (var i = 1; i < dataNota.length; i++) {
                ...
                var urlFoto = "";
                if (fotoRichText[i - 1] && fotoRichText[i - 1][0]) {
                    urlFoto = fotoRichText[i - 1][0].getLinkUrl() || "";
                }
...
function doPost(e) {
    try {
        ...
        if (data.action === "pemohon_ambil") {
            ...
            sheetP.appendRow([
                new Date(),
                sanitizeForSheet(data.tanggal),
                sanitizeForSheet(data.bbm),
                Number(data.kupon) || 1,
                sanitizeForSheet(data.plat),
                sanitizeForSheet(data.pemohon),
                false,
                "Intransit App"
            ]);
        ...
        if (data.action === "kembalikan_kupon") {
            ...
            if (sheetP && data.idPinjam && data.idPinjam.indexOf("ROW-") !== -1) {
                ...
                // Validasi batas jangkauan indeks baris (Bounds check)
                if (isNaN(rowIndex) || rowIndex <= 1 || rowIndex > lastRowP) {
                    return createJsonResponse({ status: "error", message: "ID Permohonan tidak valid atau baris tidak ditemukan." });
                }

                var kuponPinjamAwal = Number(sheetP.getRange(rowIndex, 4).getValue()) || 0;
                var jumlahRetur = Number(data.jumlahKembali) || 0;
                var alasanSanitised = sanitizeForSheet(data.alasan || "");
                ...
                if (kuponPinjamAwal > jumlahRetur && jumlahRetur > 0) {
                    ...
                    sheetP.getRange(rowIndex, 8).setValue("Dikembalikan Sebagian (" + jumlahRetur + " lbr): " + alasanSanitised);
                } else {
                    sheetP.getRange(rowIndex, 7).setValue(true);
                    sheetP.getRange(rowIndex, 8).setValue("Dikembalikan Seluruhnya: " + alasanSanitised);
                }
            }
        ...
        if (data.action === "pembelian_baru") {
            ...
            sheetBeli.appendRow([
                sanitizeForSheet(data.bbm),
                Number(data.jumlahKupon) || 0,
                sanitizeForSheet(data.bulan)
            ]);
        ...
        if (data.foto && data.foto.indexOf("data:image") !== -1) {
            ...
            var blob = Utilities.newBlob(
                ...
                "image/jpeg",
                "Nota_" + sanitizeForSheet(data.no || "BBM") + "_" + sanitizeForSheet(data.tanggal || "TGL") + ".jpg"
            );
        ...
        if (data.idPinjam && data.idPinjam.indexOf("ROW-") !== -1) {
            ...
            if (sheetP) {
                ...
                var lastRowCheck = sheetP.getLastRow();

                if (!isNaN(rIdx) && rIdx > 1 && rIdx <= lastRowCheck) {
                    ...
                    if (kuponAwal > kuponDipakai) {
                        ...
                        sheetP.getRange(rIdx, 8).setValue("Terpakai sebagian: " + kuponDipakai + " lbr (Nota " + sanitizeForSheet(data.no) + ")");
                        ...
                        sheetP.appendRow([
                            ...
                            sanitizeForSheet(data.pemohon),
                            true,
                            "LPJ Parsial Nota " + sanitizeForSheet(data.no)
                        ]);
                    } else {
                        sheetP.getRange(rIdx, 7).setValue(true);
                        sheetP.getRange(rIdx, 8).setValue("Selesai LPJ Nota " + sanitizeForSheet(data.no));
                    }
                }
            }
        }
...
function processOCR(base64Image) {
    try {
        var apiKey = getWhisperApiKey();
        if (!apiKey) {
            throw new Error("WHISPER_API_KEY belum dikonfigurasi pada Script Properties Google Apps Script.");
        }
        ...
        var statusOptions = { method: "get", headers: { "unstract-key": apiKey }, muteHttpExceptions: true };
